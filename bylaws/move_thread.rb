...
class MoveThread
  ...
  private
    ...
    def build_transaction thread_id, target_topic_id, move_thread_bylaw
      ...
      Celluloid::Actor[:eth].transact move_thread_bylaw, data
    end
    ...
    def get_values thread_id, target_topic_id
      ...
      post_position    = Celluloid::Actor[:eth].get_storage_at target_topic_id, '0x19'
      if post_position == thread_id
        @moved = true
      else
        @moved = false
      end
      @topic_id = Celluloid::Actor[:eth].get_storage_at thread_id, '0x14'
    end
end